...
npm run test --workspace=@peterson-benhame/agent-skills  # CLI tests only
...
nx g @peterson-benhame/skill-plugin:skill {name} --category={cat}  # Create new skill
```
...
- **`packages/cli`** — `@peterson-benhame/agent-skills`. The user-facing installer. Dual-mode: interactive TUI (React + Ink) and non-interactive CLI (Commander.js). Entry: `src/index.ts`. Business logic in `src/services/` (registry fetching, skill installation, lockfile management, agent configs).
...
- **`libs/core`** — `@peterson-benhame/core`. Shared types (`AgentType`, `SkillInfo`, `CategoryInfo`) and constants.
...
Always use the Nx generator (`nx g @peterson-benhame/skill-plugin:skill`) to create skills. Keep `SKILL.md` under 500 lines; offload reference material to `references/`.
