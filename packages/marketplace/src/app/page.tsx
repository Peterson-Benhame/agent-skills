...
export const metadata = buildPageMetadata({
  title: `${SITE_NAME} — Skills para agentes de programação com IA`,
            ...
            <div className="flex-1 max-w-xl">
              <div className="inline-flex items-center gap-2 bg-blue-500/6 dark:bg-blue-400/10 border border-blue-500/12 dark:border-blue-400/20 rounded-full px-4 py-1.5 mb-6">
                ...
                <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">
                  {stats.totalSkills} skills disponíveis
                </span>
              </div>

              <h1 className="text-4xl sm:text-5xl font-extrabold text-gray-900 dark:text-gray-100 leading-[1.1] tracking-tight mb-5">
                Skills para agentes de IA mais inteligentes
              </h1>

              <p className="text-base sm:text-lg text-gray-500 dark:text-gray-400 leading-relaxed mb-8 max-w-md">
                Amplie as capacidades do seu agente com skills selecionadas — workflows especializados, conhecimento de
                domínio e integrações com ferramentas.
              </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <section className="py-10">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Skills em destaque</h2>
          ...
          {heroSkill &&
            (() => {
              ...
              const heroInstallCommand = `npx @peterson-benhame/agent-skills install --skill ${heroSkill.id}`
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {featuredSkills.map((skill) => {
              ...
              const installCommand = `npx @peterson-benhame/agent-skills install --skill ${skill.id}`
        ...
        <section aria-labelledby="what-are-agent-skills" className="py-10">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed max-w-3xl mb-4">
            Uma Agent Skill é um conjunto de instruções empacotadas — um arquivo Markdown, além de scripts, templates e
            documentos de referência opcionais — que você instala em um agente de programação com IA. Depois de
            instalada, o agente carrega a skill quando a tarefa corresponde aos seus gatilhos, seguindo sempre o mesmo
            workflow revisado em vez de improvisar. Todas as skills deste registro passam por verificação de segurança e
            possuem controle de versão.
          </p>

          <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed max-w-3xl">
            Atualmente, o registro possui {stats.totalSkills} skills distribuídas em {categories.length} categorias e
            compatíveis com {stats.totalAgents} agentes de programação com IA.
          </p>
        </section>
        ...
        <section className="py-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <StatsCard label="Total de Skills" value={stats.totalSkills} icon="⚡" />
            <StatsCard label="Categorias" value={stats.totalCategories} icon="📂" />
            <NpmDownloadsCard />
          </div>
        </section>
        ...
        <section className="pb-16">
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 sm:p-8 shadow-sm">
            <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Início rápido</h2>

            <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">
              Instale skills diretamente pela linha de comando:
            </p>
            ...
            <div className="bg-slate-900 dark:bg-slate-950 rounded-xl p-4 flex items-center justify-between">
              <code className="text-sm text-sky-400 font-mono">
                npx @peterson-benhame/agent-skills install --skill [skill-name]
              </code>

              <CopyButton
                text="npx @peterson-benhame/agent-skills install --skill [skill-name]"
                className="!bg-white/10 !text-white !px-4 !py-1.5 !text-xs hover:!bg-white/20"
              />
            </div>

            <p className="text-xs text-gray-400 dark:text-gray-500 mt-4">
              As skills são instaladas no diretório de skills do seu agente (ex.: .cursor/skills/, .claude/skills/)
            </p>
          </div>
        </section>
      </div>
