...
export async function generateMetadata({ params }: { params: Promise<{ agent: string }> }): Promise<Metadata> {
  ...
  return buildPageMetadata({
    title: `${agent.name} — Instalar Agent Skills`,
    description: `Instale qualquer uma das ${catalog.stats.totalSkills} Agent Skills no ${agent.name}. As skills são instaladas em ${agent.skillsDir} no projeto ou em ${agent.globalSkillsDir} globalmente.`,
    ...
    <>
      <JsonLd
        data={graph([
          ...
          {
            '@type': 'HowTo',
            name: `Instalar Agent Skills no ${agent.name}`,
            description: `Instale skills do registro Agent Skills no ${agent.name}.`,
            url: absoluteUrl(routes.agent(agentId)),
            tool: [{ '@type': 'HowToTool', name: '@peterson-benhame/agent-skills CLI' }],
            step: [
              {
                '@type': 'HowToStep',
                name: 'Execute o instalador',
                text: `Execute ${projectCommand} na raiz do seu projeto.`,
              },
              {
                '@type': 'HowToStep',
                name: 'Verifique o local de instalação',
                text: `As skills são gravadas em ${agent.skillsDir} no projeto ou em ${agent.globalSkillsDir} com --global.`,
              },
            ],
          },
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <header className="mb-10">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-gray-100 tracking-tight mb-4">
            Agent Skills para {agent.name}
          </h1>

          <p className="text-base text-gray-500 dark:text-gray-400 leading-relaxed max-w-3xl">
            {agent.name} — {agent.description}. Todas as {catalog.stats.totalSkills} skills deste registro são arquivos
            de instruções em Markdown, portanto todas funcionam com {agent.name}; o instalador só precisa saber de onde
            o {agent.name} carrega suas skills.
          </p>
        </header>
        ...
        <section aria-labelledby="install-heading" className="mb-12">
          <h2 id="install-heading" className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">
            Como instalar skills no {agent.name}
          </h2>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
            Instalar para todos os projetos desta máquina:
          </p>
          ...
          <div className="bg-slate-900 dark:bg-slate-950 rounded-xl p-3.5 flex items-center justify-between">
            <code className="text-sm text-sky-400 font-mono truncate">
              {installCommand(exampleSkill.id, agent.id)}
            </code>
        ...
        <section aria-labelledby="paths-heading" className="mb-12">
          <h2 id="paths-heading" className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">
            Onde o {agent.name} armazena as skills
          </h2>
