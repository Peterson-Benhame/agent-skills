...
Section headings stay as they are — the next skill refers to them by name — while the prose follows the language of the document and identifiers are never translated. The document is for humans first: `pbs-implement` copies criteria into checks, and nothing under `Unresolved` gets settled while building.
...
> Build this with **pbs-implement** (`<repo-relative path, when the project vendors the skill>`).
